...
export default function UpdateProductColors({
  editedProduct,
  setEditedProduct,
  name,
  index,
  ...
  const handleColorSelection = (selectedColor) => {
    setEditedProduct((prev) => {
      const newSubProducts = [...prev.subProducts];
      const sub = { ...newSubProducts[index] };

      // sub.color is your subproduct color
      sub.color = {
        ...sub.color,
        color: selectedColor, // e.g. "#FFFFFF"
        // or sub.color?.image if you need to keep the image
      };

      newSubProducts[index] = sub;
      ...
      <>
        ...
        >
          <div
            ...
            onClick={() => {
              disabled ? "" : handleColorSelection(colorObj.color);
            }}
          ></div>
          {colorObj.name}
        </div>
  ...
  return (
    <div className={styles.colors}>
      ...
      <input
        type="text"
        value={editedProduct?.color?.color}
        name={name}
        hidden
        {...field}
        {...props}
