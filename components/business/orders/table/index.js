...
function Row(props) {
  ...
  const getDisplayDate = () => {
    ...
    const diffInDays = now.startOf("day").diff(created.startOf("day"), "day");

    // If created today or yesterday, use "fromNow()"
    if (diffInDays === 0 || diffInDays === 1) {
      return created.locale("es").fromNow();
    }

    // Otherwise, use a normal format (adjust format as you prefer)
                    ...
                    return (
                      <TableRow key={p._id}>
                        ...
                        <TableCell align="left">
                          ${p.wholesalePrice.toLocaleString("es-CO")}
                        </TableCell>
                      </TableRow>
                    );
                  })}
